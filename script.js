function handleButtonClick(isYes) {
    ...
    if (isYes) {
        ...
        sendLoveDiv.innerHTML = '<img src="dance2.gif" alt="Dance 2 GIF">';
    } else {
        // Change the gif to 'angry.gif'
        sendLoveDiv.innerHTML = '<img src="angry.gif" alt="Angry GIF">';
...
function moveButtonAway() {
    ...
    const randomX = Math.max(maxDistanceFromEdge, Math.min(maxDistanceX, Math.random() * maxDistanceX));
    const randomY = Math.max(maxDistanceFromEdge, Math.min(maxDistanceY, Math.random() * maxDistanceY));
    ...
    const duration = Math.max(0.5, Math.min(2, Math.sqrt(distanceX ** 2 + distanceY ** 2) / 100));
